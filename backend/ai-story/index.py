...
def generate_story_continuation(action: str, settings: Dict, history: List[Dict]) -> Dict[str, Any]:
    ...
    if len(history) == 0:
        if setting_description:
            enhanced_action = f"{action}\n\n⚠️ ВАЖНО: Начни историю В МИРЕ И С ПЕРСОНАЖАМИ из сеттинга игрока (см. системный промпт выше).\n🚫 НЕ создавай свой мир! Используй то, что описал игрок!\n\nНачни ярко и захватывающе! Опиши атмосферу, представь героев через действия и диалоги. Сразу создай интригу. Пиши живо, с эмоциями!"
        else:
...
def build_system_prompt(role: str, narrative_mode: str, setting: str, game_name: str) -> str:
    ...
    base += "⚠️ КРИТИЧЕСКИ ВАЖНО: Используй ТОЛЬКО тот мир и персонажей, которые описал игрок в сеттинге ниже.\n"
    base += "🚫 СТРОГО ЗАПРЕЩЕНО создавать свой мир, если игрок уже описал свой!\n"
    base += "🚫 ЗАПРЕЩЕНО использовать известные вселенные и их элементы (Mo Dao Zu Shi, Гарри Поттер, Marvel, DC, аниме, Game of Thrones и т.д.)\n\n"
    
    if setting:
        base += f"═══ СЕТТИНГ ИГРОКА (СТРОГО СЛЕДУЙ ЭТОМУ) ═══\n{setting}\n═══════════════════════════════════════════\n\n"
        base += "👆 ИСПОЛЬЗУЙ ТОЛЬКО ТО, ЧТО ОПИСАНО ВЫШЕ. Это МИР ИГРОКА, не твой!\n\n"
    else:
        base += "⚠️ Игрок не указал сеттинг. ТОЛЬКО в этом случае создай свой оригинальный мир.\n\n"
