...
def generate_story_continuation(action: str, settings: Dict, history: List[Dict]) -> Dict[str, Any]:
    ...
    if len(history) == 0:
        if setting_description:
            # КРИТИЧЕСКИ ВАЖНО: DeepSeek тупой, нужно повторить сеттинг 3 раза разными словами
            enhanced_action = f"СЕТТИНГ ИГРЫ (ОБЯЗАТЕЛЬНО К ИСПОЛНЕНИЮ):\n{setting_description}\n\n"
            enhanced_action += f"ДЕЙСТВИЕ ИГРОКА: {action}\n\n"
            enhanced_action += "🚨 АБСОЛЮТНЫЙ ЗАПРЕТ:\n"
            enhanced_action += "❌ НЕ СМЕЙ добавлять персонажей, которых нет в сеттинге выше\n"
            enhanced_action += "❌ НЕ СМЕЙ добавлять магию, культы, ритуалы если их НЕТ в сеттинге\n"
            enhanced_action += "❌ НЕ СМЕЙ менять жанр или тон истории\n"
            enhanced_action += "❌ НЕ СМЕЙ использовать клише из других вселенных\n\n"
            enhanced_action += f"✅ ТЫ ОБЯЗАН начать историю ТОЛЬКО в мире: {setting_description}\n"
            enhanced_action += "✅ Используй ТОЛЬКО элементы из сеттинга игрока\n"
            enhanced_action += "✅ Если сеттинг реалистичный - пиши реалистично\n"
            enhanced_action += "✅ Если сеттинг фантастический - используй ЕГО правила\n\n"
            enhanced_action += "Начни ярко! Опиши атмосферу. Создай интригу из элементов СЕТТИНГА ИГРОКА!"
        else:
            enhanced_action = f"{action}\n\nНачни историю ярко и захватывающе! Опиши атмосферу, представь главного героя через действия и диалоги. Сразу создай интригу или напряжение. Пиши живо, с эмоциями!"
        messages.append({'role': 'user', 'content': enhanced_action})
    else:
        # В процессе игры тоже напоминаем про сеттинг
        if setting_description and len(history) % 3 == 0:
            action_with_reminder = f"{action}\n\n(Напоминание: игра идёт в мире '{setting_description}'. Не добавляй элементы из других вселенных!)"
