...
def generate_story_continuation(action: str, settings: Dict, history: List[Dict]) -> Dict[str, Any]:
    ...
    if len(history) == 0:
        if setting_description:
            enhanced_action = f"{action}\n\n🚨 КРИТИЧЕСКИ ВАЖНО - ПЕРЕЧИТАЙ СЕТТИНГ ИГРОКА!\n"
            enhanced_action += f"Сеттинг игрока: {setting_description}\n\n"
            enhanced_action += "⚠️ Начни историю СТРОГО в этом мире! Используй ЕГО персонажей, ЕГО локации, ЕГО правила.\n"
            enhanced_action += "🚫 ЗАПРЕЩЕНО добавлять элементы из других миров или придумывать новые сущности!\n"
            enhanced_action += "✅ Если в сеттинге нет магии - НЕ добавляй магию\n"
            enhanced_action += "✅ Если нет культов - НЕ добавляй культы\n"
            enhanced_action += "✅ Придерживайся жанра и тона, которые игрок задал в сеттинге\n\n"
            enhanced_action += "Начни ярко! Опиши атмосферу, представь героев через действия и диалоги. Создай интригу. Живо, с эмоциями!"
        else:
