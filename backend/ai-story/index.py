...
def generate_story_continuation(action: str, settings: Dict, history: List[Dict]) -> Dict[str, Any]:
    ...
    if len(history) == 0:
        if setting_description:
            enhanced_action = f"{action}\n\n⚠️ Начни историю СТРОГО в мире из сеттинга игрока (выше).\nИспользуй ЕГО персонажей, ЕГО локации, ЕГО правила.\nНЕ добавляй элементы из других миров!\n\nНачни ярко! Опиши атмосферу, представь героев через действия и диалоги. Создай интригу. Живо, с эмоциями!"
        else:
...
def build_system_prompt(role: str, narrative_mode: str, setting: str, game_name: str) -> str:
    ...
    if setting:
        ...
        base += "⚠️ КРИТИЧЕСКИ ВАЖНО:\n"
        base += "✅ Используй ТОЛЬКО тот мир, персонажей и правила, которые описал игрок выше\n"
        base += "🚫 НЕ добавляй элементы из других вселенных, даже если они кажутся похожими\n"
        base += "🚫 НЕ заменяй мир игрока на известные франшизы (аниме, книги, фильмы)\n"
        base += "✅ Если игрок упомянул известную вселенную (например 'Гарри Поттер') - используй её\n"
        base += "✅ Если игрок описал СВОЙ мир - используй ТОЛЬКО его, без смешивания\n\n"
    else:
