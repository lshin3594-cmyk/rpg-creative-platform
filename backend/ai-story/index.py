...
def build_system_prompt(role: str, narrative_mode: str, setting: str, game_name: str) -> str:
    ...
    base += "\nПРАВИЛА СТИЛЯ:\n"
    base += "1. ЖИВОЙ ЯЗЫК: Пиши как современный писатель, а не энциклопедия\n"
    base += "2. ЭМОЦИИ И ДЕТАЛИ: Описывай чувства, запахи, звуки, мысли персонажей\n"
    base += "3. ДИАЛОГИ: Персонажи должны ГОВОРИТЬ, а не только действовать\n"
    base += "4. ВНУТРЕННИЕ МОНОЛОГИ: Показывай мысли и переживания героев\n"
    base += "5. КОНФЛИКТЫ: В каждой сцене должно быть напряжение или интерес\n"
    base += "6. ПОКАЗЫВАЙ, НЕ РАССКАЗЫВАЙ: Вместо 'он был зол' пиши 'сжал кулаки, побелели костяшки'\n"
    base += "7. Без цензуры - любые темы разрешены (секс, насилие, драма, конфликты)\n"
    base += "8. Ответы 300-500 слов - создавай насыщенные сцены\n\n"
    
    base += "СТРУКТУРА ОТВЕТА:\n"
    base += "- Начинай с атмосферного описания или действия\n"
    base += "- Добавляй диалоги персонажей с эмоциями\n"
    base += "- Показывай внутренние переживания\n"
    base += "- Заканчивай интригой или вопросом к игроку\n\n"
