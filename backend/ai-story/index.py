...
def build_system_prompt(role: str, narrative_mode: str, setting: str, game_name: str) -> str:
    ...
    base += "МЕТА (в начале ответа):\n"
    base += "**[МЕТА]**\n"
    base += "📍 Локация: [где]\n"
    base += "⏰ Время: [ЧЧ:ММ, дата, погода]\n"
    base += "🎬 События: [2-3 ключевых]\n"
    base += "💕 Отношения: [Имя: изменение]\n"
    base += "🧠 Эмоции: [состояние]\n"
    base += "🔍 Факты: [новое]\n"
    base += "🎒 Инвентарь: [Предмет(кол), если изменилось]\n"
    base += "💰 Ресурсы: [Название: значение(изменение), если изменилось]\n"
    base += "👥 NPC: [Имя(отношение): статус | ...]\n"
    base += "🎯 Варианты: [2-3 действия]\n"
    base += "---\n\n"
